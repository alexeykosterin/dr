...
AS
  ...
  */
  ...
  procedure inser_work_data_test;
  /**
  ...
  */
  PROCEDURE upd_customer_is_account_in_sso;
...
AS
  ...
  cursor c_TRANSFER_WAY is select rowid as row_id from idb_prod.IDB_PH2_SBPI_INT where OFF_ID_FOR_MIGR=303000018 and TRANSFER_WAY = 'Рассрочка';
  ...
  dic.IDB_TABLE_NAME = 'IDB_PH2_SBPI_INT' AND dic.TAX_SET_ID = '2') LEFT JOIN IDB_PH2_OFF_NOCHARGE_DIC noch
    ...
    WHERE  1 = 1 AND tab.IS_BASED_MRC IS NULL
    AND EXISTS (SELECT 1
...
*/
      ...
      AND (SELECT count(1)
           ...
           WHERE 1=1
             ...
             AND COALESCE(tfl.active_from, lc_current_date) <= COALESCE(alf.ACTIVE_TO, lc_current_date)
  ...
  */
  ...
  BEGIN
    ...
    -- Зачистим таблицы
    DELETE FROM idb_ph2_sbpi_int f WHERE f.source_system_type = 1 AND f.idb_id LIKE 'SUS_TI_1/%';
    DELETE FROM idb_ph2_bpi_suspend_req WHERE bpi_idb_id LIKE 'TI_1%';
    --
    ...
    INSERT INTO idb_ph2_bpi_suspend_req(bpi_idb_id, idb_id, susp_end_date, susp_start_date, susp_descr)
    SELECT t.str1, 'SUS_'||t.str1, CASE WHEN t.num2 = 1 THEN t.active_to +1 ELSE NULL END, t.active_from, t.str5
    ...
    -- Кейс классический
    INSERT INTO idb_ph2_bpi_suspend_req(bpi_idb_id, idb_id, susp_end_date, susp_start_date, susp_descr)
    SELECT t.str1, 'SUS_'||t.str1, t.active_to +1, t.active_from, NULL--Заполняется для TLO --t.str5
    FROM rias_mgr_tmp_list t
    WHERE 1 = 1
...
----===========================
  ...
  */
  ...
  BEGIN
    ...
    DELETE /*+ append */ FROM idb_ph2_access_cred ac WHERE ac.bpi_idb_id like 'TI_1/%';
    COMMIT;
    -- Сбросить ACCESS_CRED_PPPOE_ACC в IDB_PH2_SBPI_INT
    UPDATE /*+ append */ idb_ph2_sbpi_int s SET s.access_cred_pppoe_acc = NULL
    WHERE s.off_id_for_migr = '121000087' AND (idb_id LIKE 'SI_1/%' OR idb_id LIKE 'SIFI_1/%') AND access_cred_pppoe_acc IS NOT NULL;
    -- Удаляем продукт Интернет SLO
    DELETE /*+ append */ FROM idb_ph2_access_cred ac WHERE (ac.bpi_idb_id LIKE 'SI_1/%' OR ac.bpi_idb_id LIKE 'SIFI_1/%');
    COMMIT;
    ...
    WHERE s.off_id_for_migr = '121000021' AND (idb_id LIKE 'SI_1/%' OR idb_id LIKE 'SICF_1/%') AND access_cred IS NOT NULL;
    -- Удаляем продукты Интернет SLO
    DELETE /*+ append */ FROM idb_ph2_access_cred ac WHERE (ac.bpi_idb_id LIKE 'SI_1/%' OR ac.bpi_idb_id LIKE 'SICF_1/%');
    COMMIT;
...
*/
  ...
  BEGIN
    ...
    FOR city_cr IN (SELECT to_char(city_id) city_id
      ...
      with cr as (
        ...
        row_number() over (partition by addendum_id, source_system order by sbpi_rowid) rnum
          from (
                  SELECT (SELECT rowid FROM dual) as sbpi_rowid, cr.idb_id, cr.service_id, cr.auth_type, to_number(cr.source_id) as addendum_id, source_system, address_unit_id, 1 nm
                  FROM cr
                  --
                  UNION ALL
                  --
                  select sbpi.rowid as sbpi_rowid, sbpi.idb_id, sbpi.service_id, sbpi.auth_type, to_number(cr.source_id) as addendum_id, sbpi.source_system, address_unit_id, 2 nm
    ...
    LOOP
      ...
      FORALL i IN 1 .. lv_arr.count
        UPDATE idb_ph2_sbpi_int s SET s.access_cred = lv_arr(i).idb_id
         WHERE s.idb_id = lv_arr(i).bpi_idb_id;
      --dbms_output.put_line(sql%rowcount);
    END LOOP;
    CLOSE cr;
  EXCEPTION
  ...
  */
  ...
  delete from idb_ph2_net_access where substr(idb_id,1,6) = 'NEA_TI';
  commit;
  --организуем цикл по городам
  for cr_city in (
    ...
    select tbpi.incl_ip4addr assigned_ipv4addresses
     ...
     , rias_mgr_support.get_auth_type_4_net_access(tbpi.source_id, tbpi.source_system) auth_type
    ...
    and dic.off_id_for_migr not in ('121000158', '121000450')
      and tbpi.off_id_for_migr = dic.off_id_for_migr
      and tbpi.source_system = cr_city.city_id
      and tbpi.source_system_type = '1'
    ...
    (auth_type,
     ...
     source_id, source_system, source_system_type,
     service_credentials,
     cpe_mac_address, opt82_switch_mac_address, opt82_switch_port)
    ...
    , rias_mgr_support.get_auth_type_4_net_access(tbpi.source_id, tbpi.source_system) auth
      from idb_ph2_offers_oss_dic dic
      , idb_ph2_tbpi_int tbpi
      , idb_ph2_ne ne
      where 1=1
    ...
    and dic.off_id_for_migr not in ('121000158', '121000450')
     ...
     , cr.source_system_type source_system_type
       ...
       /*case
       when cr.auth IN ('PPPoE Static IP (РИАС)', 'PPPoE Dynamic IP (РИАС)', 'IPoE Дом.ру (РИАС)') THEN*/
         'AC_1' || '/' || bpi.idb_id
       /*else
         null
       end */ service_credentials,
    ...
    /*(
    select cr.idb_id idb_id, cr.service_id service_id, 1 nom, cr.source_id, cr.source_system from cr
    union
    select sbpi.idb_id, sbpi.service_id, rownum+1 nom, cr.source_id, cr.source_system from idb_ph2_sbpi_int sbpi, cr
    where 1=1
    and sbpi.parent_id = cr.idb_id
    and sbpi.source_system = cr.source_system
    and sbpi.off_id_for_migr in ('121000087', '121000102')
...
/*
    ...
    using (
    select slo.ipv6range as \*'V6RP/'||ar.billing_id||'/'||ip_v6*\ new_val, net.rowid as cr_row
      from addendum_resources_all ar
        , resource_contents_all rc
        , cable_city_ip_subnets_all lo
        , idb_ph2_net_access net
        , idb_ph2_sbpi_int slo
      where 1=1
        and substr(net.idb_id,1,6) = 'NEA_TI'
        and net.source_system = cr_city.city_id
        and net.bpi_idb_id = slo.idb_id
        and slo.off_id_for_migr = '121000093' -- V6
        and ar.billing_id = net.source_system
        and ar.addendum_id = net.source_id
        and coalesce(ar.active_to, current_date + 1) > current_date
        and rc.resource_id = ar.resource_id
        and rc.billing_id = ar.billing_id
        and coalesce(rc.active_to, current_date + 1) > current_date
        and lo.billing_id = rc.billing_id
        and lo.terminal_resource_id = rc.terminal_resource_id
        and ip_v6 is not null
    ) sour on (dest.rowid = sour.cr_row)
    when matched then update set dest.assigned_ipv6addresses = sour.new_val;
    commit;
*/
    end loop;
    ...
    where substr(r.idb_id,1,6) = 'NEA_TI'
    ...
    where substr(net.idb_id,1,6) = 'NEA_TI'
...
when n.bpi_idb_id like 'SI%' then 
  (select source_id from idb_ph2_tbpi_int inta where inta.idb_id = (select parent_id from idb_ph2_sbpi_int sinta where sinta.idb_id = n.bpi_idb_id))
...
AND PI.SERVICE_ID IN (101171,101170,101161,101169))) loop
  update idb_ph2_net_access n set n.primary_dns = '193.58.251.251',n.secondary_dns = '193.58.251.251' where idb_id = rec.idb_id;
end loop;
...
where idb_id in (select 
n.idb_id from idb_ph2_net_access n ,IP_FOR_DEDICATED_CLIENTS_ALL I,CABLE_CITY_NET_IP_POOL_ALL PA
where (bpi_idb_id like 'TI_1/%' OR bpi_idb_id like 'SIFI_1/%' OR bpi_idb_id like 'SI_1/%') and n.vlan is null
and (n.assigned_ipv4addresses is not null or n.assigned_ipv6addresses is not null) and n.primary_dns is null
and n.source_system = 556 and I.TERMINAL_RESOURCE_ID = 
(case
when n.IP_ADDRESS_ASSIGNMENT_TYPE = 'Динамический' then (select source_id from idb_ph2_ip_v4address_private pr where 
...
--2.2 шаг
UPDATE IDB_PH2_NET_ACCESS N SET
N.PRIMARY_DNS = '192.168.248.21',N.secondary_dns = '188.134.64.21'
where idb_id in (select 
n.idb_id from idb_ph2_net_access n ,IP_FOR_DEDICATED_CLIENTS_ALL I,CABLE_CITY_NET_IP_POOL_ALL PA
where (bpi_idb_id like 'TI_1/%' OR bpi_idb_id like 'SIFI_1/%' OR bpi_idb_id like 'SI_1/%') and n.vlan is null
and (n.assigned_ipv4addresses is not null or n.assigned_ipv6addresses is not null) and n.primary_dns is null
...
where (bpi_idb_id like 'TI_1/%' OR bpi_idb_id like 'SIFI_1/%' OR bpi_idb_id like 'SI_1/%') and n1.vlan is null
...
set t.ACCESS_SPEED_UP = '0'|| replace(ACCESS_SPEED_UP,',','.') where ACCESS_SPEED_UP like ',%';

update idb_ph2_tbpi_int t
set t.ACCESS_SPEED = '0'|| replace(ACCESS_SPEED,',','.') where ACCESS_SPEED like ',%';

update idb_ph2_tbpi_int t
set t.ACCESS_SPEED = replace(ACCESS_SPEED,',','.'), ACCESS_SPEED_UP =replace(ACCESS_SPEED,',','.') ;
...
begin
  delete from rias_mgr_tmp_list;
  delete from idb_ph2_bpi_suspend_req where idb_id like 'SUSP_CH%';
  idb_prod.rias_mgr_internet.inser_work_data_test;

  insert into idb_ph2_bpi_suspend_req(bpi_idb_id,
                                      idb_id,
                                      isvalid,
                                      susp_end_date,
                                      susp_start_date,
                                      susp_descr)
  select
  str1,'SUSP_CH'||str1,null,t.active_to+1,t.active_from,case when t.active_from > current_date then t.str5 else null end 
  from rias_mgr_tmp_list t
...
from idb_ph2_tbpi_int t,activate_license_fee_all alf, plan_items_all pi where t.idb_id like 'TI_1/%'
...
where 
t.idb_id like 'TI_1/%' and initial_real_cost is not null;
...
where idb_id like 'TI_1%' and (INITIAL_INSTALL_FEE = 666 OR initial_payback_period = 666);


delete from idb_ph2_productstatus where parent_id = 'TI_1/2/4814362/10140595/127918';
...
--28.05.2021
begin
  delete from idb_ph2_productstatus where idb_id like 'TI_STXNEW%';
  
  
  insert into idb_ph2_productstatus ps (CUSTOMER_IDB_ID,EFFECTIVE_DTM,
  IDB_ID,ISVALID,parent_id,PRODUCT_STATUS,source_id,source_system,source_system_type)
  select 
  s.customer_idb_id,s.actual_start_date,'TI_STXNEW'||s.idb_id,null,s.idb_id,'Active',s.source_id,s.source_system,s.source_system_type
  from idb_ph2_sbpi_int s,idb_ph2_tbpi_int t where s.parent_id = t.idb_id and t.idb_id like 'TI_1/%'
...
where rowid in( SELECT  
        IDB_PH2_SBPI_INT.rowid
                ...
                IDB_PH2_SBPI_INT.EXT_BPI_STATUS_DATE IS NULL and parent_id like 'TI%' and ext_bpi_status = 'Suspended'); 
...
s.customer_idb_id,s.actual_start_date,'TI_STXNEW'||s.idb_id,null
  ,s.idb_id,'Active',s.source_id,s.source_system,s.source_system_type  from idb_ph2_sbpi_int s,idb_ph2_tbpi_int t where s.parent_id = t.idb_id and t.idb_id like 'TI_1/%'
  and s.off_id_for_migr = 303000018
  and s.ext_bpi_status = 'Suspended' and t.ext_bpi_status = 'Suspended'
...
BEGIN
  ...
  INTO   lv_cnt
  FROM   idb_post_prod.prod9_sbpi_int t
  WHERE  1 = 1
  AND    (idb_id LIKE 'SUS_TI_1/%');

  FOR rec IN (WITH table1 AS
                 (SELECT tbpi.rowid AS tbpi_rowid,
                        ...
                        tbpi.ext_bpi_status_date,
                        tbpi.off_id_for_migr AS tbpi_off_id_for_migr,
                        source_system_type,
                        bpi_market,
                        CREATED_WHEN AS tbpi_CREATED_WHEN,
                        network AS tbpi_network,
                        CUSTOMER_LOCATION AS tbpi_CUSTOMER_LOCATION,
                        ACTUAL_START_DATE AS tbpi_ACTUAL_START_DATE,
                        mrc,
                        tax_mrc,
                        BPI_ORGANIZATION AS tbpi_BPI_ORGANIZATION,
                        tbpi.parent_id,
                        REGEXP_SUBSTR(tbpi.IDB_ID, '[^/]+', 1, 3) AS agreement_id,
                        REGEXP_SUBSTR(tbpi.IDB_ID, '[^/]+', 1, 4) AS addendum_id
                 FROM   idb_prod.idb_ph2_tbpi_int tbpi)
                SELECT /*+parallel (4)*/
                DISTINCT pi.plan_item_id,
                         ad.addendum_id,
                         ad.addendum_number,
                         ad.billing_id,
                         ...
                         ag.agreement_id,
                         (SELECT city_name
                          FROM   excellent.cities_all ca
                          WHERE  ca.city_id = ad.billing_id
                         ...
                         tbpi_idb_id,
                         ext_bpi_status_date,
                         tbpi_off_id_for_migr,
                         tbpi_CREATED_WHEN,
                         tbpi_CUSTOMER_LOCATION,
                         t.source_system_type,
                         t.bpi_market,
                         tbpi_ACTUAL_START_DATE,
                         tbpi_BPI_ORGANIZATION,
                         tbpi_NETWORK
                FROM   table1                             t,
                       excellent.plan_groups_all          pg,
                       excellent.plans_all                pa,
                ...
                WHERE  1 = 1
                AND    t.addendum_id = ad.addendum_id
                AND    t.agreement_id = ag.agreement_id
                AND    t.source_system = ag.billing_id
                AND    pg.product_id = 7
                AND    pg.plan_group_id IN (41, 86, 77)
                AND    tla.addendum_id = ad.addendum_id
                AND    tla.billing_id = ad.billing_id
                AND    tla.active_from <= current_date
                AND    (tla.active_to IS NULL OR
                      tla.active_to > current_date)
                AND    t.teo_id = tla.teo_id
                AND    t.billing_id = tla.billing_id
                AND    pp.point_plugin_id = t.point_plugin_id
                AND    pp.billing_id = t.billing_id
  ...
  LOOP
    BEGIN
      ...
      ;
      UPDATE idb_prod.idb_ph2_tbpi_int tbpi
      ...
      IF lv_idb_PRODUCTSTATUS IS NOT NULL THEN
        ...
        WHERE  ips.PARENT_ID = lv_idb_PRODUCTSTATUS;
      END IF;
    
      INSERT INTO IDB_PH2_PRODUCTSTATUS
      ...
      INSERT INTO IDB_PH2_SBPI_INT
        SELECT NULL ACCESS_CRED,
               NULL ACCESS_CRED_PPPOE_ACC,
               NULL ACCESS_MODE,
               NULL ACCESS_MODE_NOTES,
               NULL ACCESS_SPEED,
               NULL ACCESS_SPEED_LOCAL_DOWN,
               NULL ACCESS_SPEED_LOCAL_UP,
               rec.ACCOUNT_IDB_ID ACCOUNT_IDB_ID,
               ...
               NULL ISVALID,
               NULL IS_BASED_MRC,
               NULL LEGACY_OBJECT_NAME,
               NULL MAIN_INTERNET_SERVICE_ID,
               ...
               NULL OTS_STATUS,
               rec.tbpi_idb_id PARENT_ID,
               NULL PARENT_SERVICE_ID,
               NULL PERIODIC_CHARGE_ID,
               NULL PREFIX,
               NULL PREFIX_SIZE,
               NULL PRIMARY_STATUS_REASON,
               NULL PROTECTED_OBJ_IP6,
               NULL PURCHASE_PRICE,
               NULL RATE_PLAN_ACTIVATION_DATE,
               NULL RATE_PLAN_CODE,
               NULL RATE_PLAN_ID,
               NULL RATE_PLAN_NAME,
               NULL SECOND_STATUS_REASON,
               CASE
                 WHEN rec.tbpi_off_id_for_migr = 121000390 THEN
                  'INTINDSUSP-' || to_char(lv_ACTUAL_START_DATE_16$d, 'ddmmyyyy') || '1' ||
                  lv_cnt
                 WHEN rec.tbpi_off_id_for_migr = 121000879 THEN
                  'INTSPEEDPLSUSP-' || to_char(lv_ACTUAL_START_DATE_16$d, 'ddmmyyyy') || '1' ||
                  lv_cnt
                 WHEN rec.tbpi_off_id_for_migr = 121000837 THEN
                 ...
                 WHEN rec.tbpi_off_id_for_migr = 121000121 THEN
                  ...
                  lv_cnt
                 WHEN rec.tbpi_off_id_for_migr = 121000296 THEN
                  'INTWIUNLIMSUSP-' || to_char(lv_ACTUAL_START_DATE_16$d, 'ddmmyyyy') || '1' ||
                  lv_cnt
                 WHEN rec.tbpi_off_id_for_migr = 121000355 THEN
                  'INTMOUNLIMSUSP-' || to_char(lv_ACTUAL_START_DATE_16$d, 'ddmmyyyy') || '1' ||
                  lv_cnt
               END service_id,
               ...
               NULL SUBNET,
               NULL SUBNET_ADD,
               NULL SUSPENDED_PERIODIC_CHARGE_ID,
               NULL TARIFF_ID,
               NULL TARIFF_PLAN_NAME,
               0 tax_mrc,
               NULL TOTAL_PRICE_TAX_NRC,
               NULL TRANSFER_WAY,
               NULL UNINSTL_DATE,
               NULL UP_ON,
               NULL UP_TO,
               NULL VENDOR,
               'N' BARRING,
               NULL INDEXATION_DATE,
               NULL PROJECT,
               NULL MRC_CUPON_MNT,
               NULL NRC_CUPON_MNT,
               NULL SUSPEND_REASON,
               NULL SUSPEND_REASON_DESCR,
               NULL NC_INTERNAL_TLO_IDB_ID,
               NULL EQUIP_CONDITION,
               NULL NRC_CUPON,
               NULL DURATION_WARRANTY,
               NULL WARRANTY_TO,
               NULL EXTRA_CHARGE_FOR_RENT,
               NULL EXTRA_CHARGE_FOR_SALE,
               NULL PRICE_RATIO
        FROM   dual;
...
INSERT INTO idb_ph2_bpi_mrc_priceoverride
  ...
  FROM   dual;
      ...
      INSERT INTO idb_prod.aak_sbpi_int
...
WITH service_table_103257 AS
  ...
  where REGEXP_SUBSTR(sbpi.IDB_ID , '[^/]+' , 1 , 4 ) = ad.addendum_id
  AND    alf.billing_id = ad.billing_id
  AND    alf.addendum_id = ad.addendum_id
...
with t_INDEXATION_DATE as (SELECT count(1) over (partition by tfl.teo_id, tfl.billing_id) as ct, 
           ...
           where tlaa.teo_id = tid.teo_id and tlaa.billing_id = tid.billing_id
...
where incl_ip4addr like 'Неизвестный%';
...
where s.assigned_ipv4addresses like '%Неизвестный%'/*  and s.off_id_for_migr = 121000087*/;
  ...
  */
  ...
  BEGIN
    ...
    WHERE 1=1
      ...
      AND ar.active_from <= rias_mgr_support.get_current_date
      AND (ar.active_to IS NULL OR ar.active_to > rias_mgr_support.get_current_date)
      AND rc.resource_id = ar.resource_id
      AND rc.billing_id = ar.billing_id
      AND rc.active_from <= rias_mgr_support.get_current_date
      AND (rc.active_to IS NULL OR rc.active_to > rias_mgr_support.get_current_date)
    ...
    */
    ...
    UPDATE /*+ append */ idb_ph2_tbpi_int tab
     ...
     WHERE tab.rowid IN (SELECT tlo.rowid
                          ...
                          WHERE tl.task_id = lv_task_2_id$i
                            AND str2 LIKE 'TI_1/%'
...
*/
      ...
      LOOP
        ...
        FORALL i IN 1 .. lv_arr.count
          UPDATE /*+ append */ idb_ph2_net_access t
           ...
           WHERE t.idb_id = lv_arr(i).idb_id;
        COMMIT;
      END LOOP;
      CLOSE crs_data_na;
    EXCEPTION
  ...
  */
  ...
  BEGIN
    -- 0. Очисить таблицу
    DELETE /*+ append */ FROM idb_ph2_customer_users WHERE source_system_type = '1';
    COMMIT;
...
*/
  ...
  */
  ...
  IS
  BEGIN
    UPDATE /*+ append */ idb_ph2_customer SET is_account_in_sso = NULL WHERE source_system_type = '1';
    COMMIT;
    --
...
update idb_ph2_sbpi_int s set s.mrc =0, s.tax_mrc = 0 where SOURCE_ID = rec.source_id and SERVICE_ID = rec.service_id and OTS_STATUS is null;
end loop;
end;
...
select round((cnt_ip*COST_IP)/(cnt_ip_bss+sum_prefix),2) as MAGIC, t.* from table6 t
), table8 as (
...
loop
  if rec.MAX_MAGIC is not null then  ---если PREFIX is null для 121000087, то использовать параметр из off_id_for_migr=121000102
     select rec.MAX_MAGIC into v_cen from dual;
  else
     select rec.COST_IP into v_cen from dual; ---если совсем нет PREFIX
  end if;
  update idb_ph2_sbpi_int set MRC = v_cen * power(2,32-rec.PREFIX)*100000, 
  TAX_MRC = (round(v_cen * power(2,32-rec.PREFIX) /1.2 , 2)-v_cen * power(2,32-rec.PREFIX))*(-1)*100000
  where PARENT_ID = rec.parent_id and off_id_for_migr=121000102
  and SERVICE_ID = rec.SERVICE_ID;
  
  
  if rec.cnt_ip_bss > 0 then
  update idb_ph2_sbpi_int set MRC = v_cen*100000, 
  TAX_MRC = (round(v_cen /1.2, 2)-v_cen)*(-1)*100000
